...
bool VideoCap::open(const char *url) {
    ...
    this->video_dec_ctx->thread_count = 16;
    ...
    while(!valid) {
        ...
        if(got_frame) {
            ...
            if (this->is_rtsp && packet.synced) {
                ...
                double rtp_diff = (double)(packet.timestamp - packet.last_rtcp_timestamp) / 90000.0 - 0.15;
...
bool VideoCap::retrieve(uint8_t **frame, int *width, int *height, double *frame_timestamp) {
...
bool VideoCap::read(uint8_t **frame, int *width, int *height, double *frame_timestamp) {
    bool ret = this->grab();
    if (ret)
        ret = this->retrieve(frame, width, height, frame_timestamp);
    return ret;
}
